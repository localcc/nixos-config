{
  inputs,
  pkgs,
  config,
  ...
}:
let

  homeOptions = { name, ... }: {
    home = "/Users/${name}";
  };

  nixosOptions = { keys, groups, ... }: {
    extraGroups = groups;
    group = "users";
    isNormalUser = true;
  };

  user = params@{ keys, groups, uid, shell, name }: {
    inherit uid shell;
    openssh.authorizedKeys.keys = keys;
  } // (if pkgs.stdenv.isDarwin then (homeOptions params) else (nixosOptions params));
in
{
  age.secrets.kate-password.file = (inputs.secrets + /kate-password.age);

  users.users = {
    kate = user {
      name = "kate";
      # Slight fuckup
      uid = if pkgs.stdenv.isDarwin then 501 else 1000;
      shell = pkgs.nushell;
      keys = [
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEHWfBIxvY4c0Rdava/cAEa3qGUOxMSt4Cu0Ap7RtSK7"
          "ssh-rsa "
      ];
      groups = 
        [ "wheel" "networkmanager" "libvirtd" "kvm" "samba" "podman" "wireshark" "audio" ];
    };
  };
  # users.mutableUsers = false;
  nix.settings.trusted-users = [ "kate" ];

  users.groups.users.gid = 100;
}
